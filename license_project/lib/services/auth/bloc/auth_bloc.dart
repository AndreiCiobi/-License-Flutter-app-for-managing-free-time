...
import 'package:license_project/services/auth/auth_provider.dart';
...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  AuthBloc(AuthProvider provider)
    ...
    on<AuthEventLogInAfterVerification>(
      (event, emit) async {
        try {
          final isEmailVerified = await provider.checkEmailVerification();
    ...
    on<AuthEventForgotPassword>(
      (event, emit) async {
        ...
        final email = event.email;
        if (email == null) {
          return; // user just wants to go to forgot-pass screen
        }

        // user wans to actually send a forgot-pass email
        emit(
          const AuthStateForgotPassword(
            exception: null,
            hasSentEmail: false,
